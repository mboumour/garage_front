...
export default function Appointment() {
  ...
  const handleDateSelect = (date) => {
    setFormData({ ...formData, preferred_date: date, preferred_time: '' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    await base44.integrations.Core.SendEmail({
      ...
      subject: `Nouveau RDV: ${formData.client_name} - ${services.find(s => s.value === formData.service)?.label}`,
      body: `
        ...
        Date souhaitée: ${formData.preferred_date ? format(formData.preferred_date, 'EEEE d MMMM yyyy', { locale: fr }) : 'Non précisée'}
